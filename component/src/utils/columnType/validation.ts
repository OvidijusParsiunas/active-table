...
export class Validation {
  ...
  public static setSelectValidation(type: ColumnTypeInternal, isDefaultTextRemovable: boolean, defaultText: CellText) {
    if (!type.cellDropdownProps?.options || type.cellDropdownProps?.canAddMoreOptions) return;
    const optionsMap = new Set<CellText>(type.cellDropdownProps.options.map((option) => option.text));
    type.textValidation ??= {};
