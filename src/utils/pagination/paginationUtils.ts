...
export class PaginationUtils {
  ...
  private static getRelativeRowDetails(paginationInternal: PaginationInternal, rowIndex: number) {
    ...
    return {maxVisibleRowIndex, visibleRowIndex};
  }
  ...
  private static updateRowsOnRemoval(etc: EditableTableComponent, rowIndex: number) {
    ...
    const {visibleRowIndex} = PaginationUtils.getRelativeRowDetails(etc.paginationInternal, rowIndex);
    visibleRows.splice(visibleRowIndex % rowIndex, 1);
    if (visibleRows.length > 0) {
      ...
      PaginationUtils.setCorrectRowsAsVisible(etc, activeButtonNumber - 1);
    }
  }
  ...
  private static updateRowsOnNewInsert(etc: EditableTableComponent, rowIndex: number, newRowElement: HTMLElement) {
    ...
    const {maxVisibleRowIndex, visibleRowIndex} = PaginationUtils.getRelativeRowDetails(etc.paginationInternal, rowIndex);
