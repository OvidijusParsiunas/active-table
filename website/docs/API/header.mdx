...
Properties related to the data content.
...
The header row will remain at the top of the table when there is overflow in the table or the direct parent element.
...
<script>
  ...
  activeTableElementRef.setAttribute('updateCellText', JSON.stringify({'newText', 1, 1}));
</script>
```
...
Checks table content for spelling mistakes (based on the browser's language) and if found displays a curly red line under the errors.
